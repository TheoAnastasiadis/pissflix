            ...
            try {
                ...
                if (response.status == 200) {
                    ...
                    return new result_1.Result(true, undefined, new movie_entity_1.Movie({
                        ...
                        languages: data.spoken_languages.map((language) => new subtypes_1.Language(language.iso_639_1, "639-1")),
                        countries: data.production_countries.map((country) => new subtypes_1.Country(country.iso_3166_1)),
                    }));
                }
                else {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const response = yield api.get(baseURL +
                    `discover/movie/?primary_release_date.gte=${startDate.toLocaleDateString("en-US").replace(/(\d{1,2})\/(\d{1,2})\/(\d{4})/gm, "$3")}&primary_release_date.lte=${endDate.toLocaleDateString("en-US").replace(/(\d{1,2})\/(\d{1,2})\/(\d{4})/gm, "$3")}` //TMDB API bug: 2020-01-01 --> no results, 2020 --> 20 results
                );
                console.log(baseURL +
                    `discover/movie/?primary_release_date.gte=${startDate.toLocaleDateString("en-US").replace(/(\d{1,2})\/(\d{1,2})\/(\d{4})/gm, "$3-$1-$2")}&primary_release_date.lte=${endDate.toLocaleDateString("en-US").replace(/(\d{1,2})\/(\d{1,2})\/(\d{4})/gm, "$3-$1-$2")}`);
