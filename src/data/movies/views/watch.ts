...
const getOneofResolution : (entries: TorrentT[], repo: DebridProviderRepo) => TO.TaskOption<TorrentT> = (entries: TorrentT[], repo: DebridProviderRepo) => pipe(
    TO.tryCatch(
        () => any(entries.map(torrent => repo.checkIfAvailable(torrent.magnetURI)))),
    TO.map((magnet) => entries.filter((torrent) => torrent.magnetURI == magnet)),
    TO.chain(TO.fromPredicate((torrent) => torrent.length > 0)),
    TO.map((torrents) => torrents[0])
)

const getOneOfEachResolution : (repo: DebridProviderRepo) => (torrents: TorrentT[]) => TE.TaskEither<string, TorrentT[]> = (repo) => (torrents) => pipe(
    torrents,
    ...
    TE.fromTaskOption(() => 'Unofrtunately no links are available for this title from your Debrid Provider. Try local play instead.')
    )
...
) => TE.TaskEither<MsxContentRoot, MsxContentRoot> = (imdbId, title, torrentRepo, debridRepo) =>
    pipe(
        torrentRepo.getTorrentsByImdbId(imdbId),
...
export const watchView: View<{ torrentRepo: TorrentRepo, debridRepo: DebridProviderRepo }, typeof watchParams> =
        ...
        pipe(
            ...
            E.map((query) =>
                pipe(
                    ...
                    E.map((query) =>
                        toRemoteContent(query.imdbId, query.title, context.torrentRepo, context.debridRepo)
                    ),
                    E.getOrElse((query) =>
                        toLocalContent(query.imdbId, query.title, context.torrentRepo)
                    )
                )
            ),
