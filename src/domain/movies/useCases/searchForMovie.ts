...
export const searchForMovie = (query: string) => (pagination: paginationParamsT) => (repo: MoviesRepoT) => pipe(
    query,
    O.fromPredicate(s => s.trim().length > 0),
    O.map(trimmedString => repo.findMany({query: trimmedString}, pagination)),
