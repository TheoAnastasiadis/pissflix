...
describe("watch view", () => {
    test("with local player", async () => {
        const content = (await watchView.render(mockedContext)(watchParams)({imdbId:'tt123456', player: "local", title: "Title"})() as E.Right<MsxContentRoot>).right

        expect(content.items).toHaveLength(10)
    })
    test("with remote player", async () => {
        const content = (await watchView.render(mockedContext)(watchParams)({imdbId:'tt123456', player: "remote", title: "Title"})() as E.Right<MsxContentRoot>).right
